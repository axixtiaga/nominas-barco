...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const old = await prisma.species.findUnique({ where: { id } });
  if (!old) return apiError("No encontrado", 404);
  const updated = await prisma.species.update({ where: { id }, data: parsed.data });
  await createAuditLog({ userId: session.id, action: "UPDATE", entity: "Species", entityId: id, oldValues: old, newValues: updated });
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!session) return apiUnauthorized();
  const { id } = await params;
  await prisma.species.update({ where: { id }, data: { active: false } });
  await createAuditLog({ userId: session.id, action: "DELETE", entity: "Species", entityId: id });
