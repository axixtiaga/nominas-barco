...
export async function getDashboard(params: { from?: Date; to?: Date } = {}) {
  ...
  const [totalInvoices, aggInv, byPort, byBoat, bySupplier, byMonth, bySpecies, byRaw] = await Promise.all([
    ...
    prisma.invoice.aggregate({ where, _sum: { subtotal: true, taxes: true, fees: true, total: true }, _avg: { total: true } }),
    prisma.invoice.groupBy({ by: ["portId"], where, _count: { _all: true }, _sum: { total: true } }),
    prisma.invoice.groupBy({ by: ["boatId"], where, _count: { _all: true }, _sum: { total: true } }),
    prisma.invoice.groupBy({ by: ["supplierId"], where, _count: { _all: true }, _sum: { total: true } }),
    prisma.$queryRawUnsafe<any[]>(
      `SELECT to_char(date_trunc('month', "issueDate"), 'YYYY-MM') as month,
              COUNT(*)::int as invoices, COALESCE(SUM("total"),0)::float as total
         FROM "Invoice"
  ...
  return {
    ...
    byPort: byPort.map(r => ({ portId: r.portId, portName: name(ports, r.portId) ?? "—", invoices: r._count._all, total: Number(r._sum.total ?? 0) })),
    byBoat: byBoat.map(r => ({ boatId: r.boatId, boatName: name(boats, r.boatId) ?? "—", invoices: r._count._all, total: Number(r._sum.total ?? 0) })),
    bySupplier: bySupplier.map(r => ({ supplierId: r.supplierId, supplierName: name(suppliers, r.supplierId) ?? "—", invoices: r._count._all, total: Number(r._sum.total ?? 0) })),
